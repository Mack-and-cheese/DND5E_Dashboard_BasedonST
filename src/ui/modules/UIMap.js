...
export default {
    ...
    async renderMiniMap($el) {
        ...
        if (!isCombat) {
            ...
            // 【核心修改】：彻底移除原本的 await ExplorationMapManager.getMap(...) API调用
            const safeLocationArg = JSON.stringify(locationName);
            $innerMap.html(`
                <div style="text-align:center;color:var(--dnd-text-dim);padding:25px 15px;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;">
                    <div style="font-size:32px;margin-bottom:8px;opacity:0.6;"><i class="fa-solid fa-map-location-dot"></i></div>
                    <div style="font-size:13px;font-weight:bold;color:var(--dnd-text-highlight);margin-bottom:4px;">${locationName}</div>
                    <div style="font-size:11px;color:#888;margin-bottom:12px;">探索模式已停用自动 AI 绘图</div>
                    <button type="button" onclick='window.DND_Dashboard_UI.regenerateMap(${safeLocationArg}, "svg")' style="background:var(--dnd-bg-tertiary);border:1px solid var(--dnd-border-gold);color:var(--dnd-text-main);border-radius:4px;padding:5px 12px;cursor:pointer;font-size:11px;transition:all 0.2s;" onmouseover="this.style.background='var(--dnd-selected-bg)'" onmouseout="this.style.background='var(--dnd-bg-tertiary)'">
                        <i class="fa-solid fa-palette"></i> 手动生成此场景地图
                    </button>
                </div>
            `);
            return;
